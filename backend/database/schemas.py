...
class ReviewSchema(ma.Schema):
    ...
    user = ma.Nested(UserSchema, many=False)
    class Meta:
        fields = ("id", "book_id", "comment", "rating", "user_id", "user")

    @post_load
    def create_review(self, data, **kwargs):
        return Review(**data)
rewiew_schema = ReviewSchema()
...
class FavoriteSchema(ma.Schema):
    ...
    book_id = fields.String()
    title = fields.String()
    thumbnail_nail = fields.String()
    user_id = fields.Integer()
    user = ma.Nested(UserSchema, many=False)
    class Meta:
        fields = ("id", "book_id", "title", "thumbnail_nail", "user_id", "user")
    @post_load
